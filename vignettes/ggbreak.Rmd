...
For creating gap plot, we only provide `scale_x_break` and `scale_y_break` functions. Currently, it is not allowed to apply both functions to set breakpoints for both x and y axes. However, multiple breakpoints on a single axis is supported.
...
1. The features we introduced for `scale_x_break` and `scale_y_break` also work for `scale_wrap`, `scale_x_cut` and `scale_y_cut`.
2. `xlim` is not compatible with `ggbreak`. Instead, users can use [expand_limits](https://github.com/YuLab-SMU/ggbreak/issues/22). 
