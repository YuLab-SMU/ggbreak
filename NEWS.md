...
+ the axis title of a broken plot is now drawn exactly once, whatever it comes from; the #85 entry of 0.2.0 was about the defect #53 fixed and assumed a title set with `labs()`, and the report came back for a title given as the `name` of a position scale -- `scale_x_continuous("test")` -- where five things went wrong together. The labels of the figure were read off `ggplot_build(plot)$plot$labels`, which only holds what `labs()` set plus the aesthetic defaults, so a plot whose scale is named "test" reported its title as "x" and the copy hoisted above a bottom legend carried the wrong text; `ggplot2::get_labs()` is used now, which returns the completed labels. A window blanked its title with `guides(x = guide_axis(title = NULL))`, and that does not beat the `name` of a scale, so `scale_wrap()` and the cut scales printed the title once per window; the theme element is blanked instead, and only on the windows, so the figure still draws it. `check_axis_title()` added the title to the outer ggplot a second time as a guide axis, whose row sits *below* the collected legend, which is why a scale-named title came out twice, once above the legend and once below it; only the secondary-axis title is left there, because `labs()` cannot set it. `grid.draw.ggwrap()` wrote the hoisted label back with `totallabs$x <- NULL`, and assigning `NULL` drops the element rather than clearing it, so a wrap drew a stray "x" under its legend. And the title is moved out of the panel by `align_assembled_figure()`, so a broken plot with a legend at the bottom keeps it when `patchwork` or `cowplot` take it through `ggplotGrob()` instead of being handed over titleless (2026-09-22, Tue, #85)
...
+ a plot that carries two of the scales `ggbreak` adds no longer draws something wrong; `scale_wrap(n = 2) + scale_y_break(...)` came out as a rendering of the break nested inside the windows of the wrap, because `grid.draw()` strips only its own class and hands the subplots on with the other one still on them, and `aplot::plot_list()` does not treat a plot with one of these classes as a plain plot -- `ggfun::is.ggbreak()` is TRUE for `ggbreak`, `ggwrap` and `ggcut`, so it calls `ggbreak2ggplot()` and *draws* it. The only sign of it was a "Coordinate system already present" message. A plot can be broken on both axes and one axis can be cut, so the pairs that cannot be drawn now say so and name the two scales; a cut on both axes is one of them, and it was worse than unsupported, because a cut is stored in a single attribute, so the second cut replaced the first and the figure was byte-identical to the single cut, with nothing said about the cut that was dropped (2026-09-20, Sun, #31)
+ a broken plot no longer hands the geometry its panels hide to the device; a break is drawn by clipping, so every window is given the whole dataset and the panel hides the part that belongs to the other windows, but the hidden geometry is still drawn into the file -- a bar from 0 to 990 is a rectangle as tall as the unbroken axis in both windows, and the window that shows its top carries one reaching about 1.8 panel heights above the panel it is drawn in, which a vector device writes out and Illustrator then refuses to paste ("the requested transformation would make some objects too large"). A rectangle, a segment that runs along the axis and a rectangle drawn as a polygon are now cut back to the edge of the panel they are drawn in; the panel clips that part away anyway, so nothing is drawn differently (0 pixels over the whole rendering battery, and over the two figures of the report), and a line is left alone because the piece it hides is read off the grob to bridge the break (2026-09-20, Sun, #16)
